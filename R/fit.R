...
sdmTMB <- function(
  ...
  # Determine what to collapse based on control$collapse
  should_collapse <- FALSE
  check_variance <- FALSE
  check_correlation <- FALSE

if (is.logical(control$collapse)) {
  if (control$collapse) {
    should_collapse <- TRUE
    check_variance <- TRUE
    check_correlation <- TRUE
  }
} else if (control$collapse == "variance") {
  should_collapse <- TRUE
  check_variance <- TRUE
} else if (control$collapse == "correlation") {
  should_collapse <- TRUE
  check_correlation <- TRUE
}

# Run unified check if any collapsing is enabled
if (should_collapse && length(tmb_obj$par) > 0) {
    report_vals <- tmb_obj$report()
  
  # Only compute sd_report if we're checking correlation
  # (variance checks don't need it)
  sd_report <- NULL
  if (check_correlation) {
    sd_report <- tryCatch({
      TMB::sdreport(tmb_obj, getJointPrecision = FALSE)
    }, error = function(e) {
      if (!silent) {
        message("Could not compute standard errors for correlation checks")
      }
      NULL
    })
  }
  
  collapse_result <- check_and_collapse(
    tmb_obj = tmb_obj,
    ...
    collapse_variance_threshold = control$collapse_variance_threshold,
    collapse_correlation_threshold = control$collapse_correlation_threshold,
    delta = delta,
    silent = silent,
    sd_report = sd_report  # Pass it in
  )
  ...
  if (collapse_result$do_refit) {
    if (!silent) {
      cli_inform(c(
        "i" = "Refitting model with updated structure using update()..."
      ))
    }
    
    # Update with ALL potentially modified arguments
    updated_fit <- update(
      out_structure,
      spatial = collapse_result$spatial_arg,
      spatiotemporal = collapse_result$spatiotemporal_arg,
      time_varying_type = collapse_result$time_varying_type_arg,  # NEW!
      do_fit = TRUE,
      silent = silent
    )
    return(updated_fit)
  }
  # STORE the warning info for later (if no refit happened)
  final_warning <- collapse_result$warning_info
}
  ...
  # Display any warnings from collapse checks
if (exists("final_warning") && !is.null(final_warning) && !silent) {
  cli_inform(c(
    "!" = final_warning$message
  ))
}
  out
}
